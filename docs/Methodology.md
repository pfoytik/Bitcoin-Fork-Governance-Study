...
### 13.5 Preliminary Findings (March 2026)

#### Data Coverage

| Regime | Scenarios | Sweeps | v27 Win Rate |
|--------|-----------|--------|--------------|
| 144-block | 232 | 7 | 44.0% |
| 2016-block | 63 | 6 | 54.0% |

#### Feature Importance

Random forest feature importance (fraction of total):
...
*Zero importance in 2016-block due to fixed values in primary sweep.

**Key finding:** `economic_split` dominates outcome prediction in both regimes.
`pool_committed_split` importance nearly triples at 2016-block (29% vs 12%),
suggesting pool commitment becomes more critical when difficulty adjusts slowly.

#### PRIM Uncertainty Bounds (144-block)

The region of maximum outcome uncertainty (closest to 50/50):
...
- Support: 87.9% of samples fall within these bounds
...
This may relate to the "Foundry flip-point" identified in Phase 1: when the
largest neutral pool (Foundry, ~30% hashrate) flips to v27, it can trigger
economic node switching that disadvantages v27. The mechanism requires further
investigation.
...
| RF accuracy | 83.8% | 74.1% | -9.7% |
| Mean contentiousness | 0.275 | 0.279 | +0.004 |

The higher v27 win rate at 2016-block suggests longer retarget periods may
favor the stricter fork, though this finding requires validation with more
...
1. **Validate fixed parameters at 2016-block**: Run verification sweep to confirm
   non-causality findings hold at slower retarget intervals

2. **Expand 2016-block coverage**: Current 63 scenarios have limited parameter
   diversity; need full 4-parameter grid

3. **Phase 3 LHS sampling**: Generate Latin Hypercube samples within PRIM bounds
   for focused uncertainty exploration

4. **Investigate inversion zone**: Determine mechanism behind the paradoxical
   effect of pool commitment in the 0.60–0.70 economic split range
